...
final class Descriptor<T> {
    ...
    Descriptor(Class<T> clazz, List<BinaryRelation<T, ?>> registeredRelations) {
        ...
        try (InputStream in =
            ...
            new ClassReader(in)
                    .accept(
                            new ClassAnalyzer<>(clazz, fieldMap),
        ...
        for (Map.Entry<BinaryRelation<T, ?>, Field> entry : fieldMap.entrySet()) {
            accessorMap.put(
                    entry.getKey(),
                    new BoundReferenceHolderAccessor(entry.getKey(), entry.getValue()));
        }
        ...
        if (boundReferenceHolderAccessors.isEmpty()) {
            ...
            referenceHolderSetAccessor =
                    new ReferenceHolderSetAccessor(boundReferenceHolderAccessors);
            for (BinaryRelation<T, ?> relation : registeredRelations) {
                if (!accessorMap.containsKey(relation)) {
                    accessorMap.put(
                            relation,
                            new UnboundReferenceHolderAccessor(
                                    referenceHolderSetAccessor, relation));
                }
            }
        }
    }
